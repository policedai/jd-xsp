...
def create_hd_text_image(title, content, font_path, settings):
    ...
    hd_img = PIL.Image.new('RGBA', (canvas_w, total_h + 100), (0, 0, 0, 0))
...
def process_audio_only(content, client, voice_id):
    ...
    out = "pure_audio_output.mp3"
    final_audio.write_audiofile(out, fps=44100, logger=None)
    for f in temp_files:
...
def process_video_full(title, content, bg_source, client, settings):
    ...
    try:
        ...
        # 【关键修改】将文字位置设为 (center, 60)，让内容靠顶显示
        text_clip = ImageClip(text_arr).set_duration(duration).set_position(('center', 60))
...
def main():
    ...
    with tab_video:
        with st.expander("⚙️ 样式设置", expanded=True):
            ...
            with v_col1:
                ...
                bg_mode = st.radio("🖼️ 背景", ["上传图片", "纯色背景"], horizontal=True, key="v_bg_m")
            with v_col2:
                t_color = st.selectbox("🎨 文字", ["鹅黄色", "科技白", "象牙金", "护眼绿", "天空蓝"], key="v_c")
                c_size = st.slider("📝 正文", 20, 60, 34, key="v_s")
                bg_color = st.selectbox("🎨 背景色", ["深邃黑", "莫兰迪灰", "奶茶色", "暗夜绿", "磨砂蓝"]) if bg_mode == "纯色背景" else "深邃黑"
            v_need_mp3 = st.checkbox("顺便导出 MP3", value=True, key="v_mp3")

        st.markdown("---")
        v_title = st.text_input("💎 标题:", "输入标题", key="v_title_in")
        v_content = st.text_area("✍️ 正文:", height=250, key="v_cont_in")
        v_bg_file = st.file_uploader("📸 背景:", type=["jpg", "png", "jpeg"]) if bg_mode == "上传图片" else None
        ...
        if st.session_state.v_path:
            ...
            with d_col1:
                with open(st.session_state.v_path, "rb") as f: st.download_button("📥 下载 MP4", f, "video.mp4", use_container_width=True)
            if v_need_mp3 and st.session_state.a_path:
                with d_col2:
                    with open(st.session_state.a_path, "rb") as f: st.download_button("🎵 下载 MP3", f, "audio.mp3", use_container_width=True)
    ...
    with tab_audio:
        ...
        if st.button("🎵 立即生成 MP3", use_container_width=True):
            st.session_state.pure_a_path = None
        ...
        if st.session_state.pure_a_path:
            ...
            with open(st.session_state.pure_a_path, "rb") as f: st.download_button("📥 下载 MP3", f, "pure_audio.mp3", use_container_width=True)
